...
<div id="{self.id}" class="modal" style="display: none; position: fixed; z-index: 1001; left: 0; top: 0; width: 100%; height: 100%; overflow: auto; background-color: rgba(0,0,0,0.4); {self.style}">
    <div class="modal-content" style="background-color: #fefefe; margin: 15% auto; padding: 20px; border: 1px solid #888; width: 80%;">
        <div class="modal-header" style="display: flex; justify-content: space-between; align-items: center;">
            <h2>{self.title}</h2>
            <span class="close" onclick="closeModal('{self.id}')" style="color: #aaa; float: right; font-size: 28px; font-weight: bold; cursor: pointer;">&times;</span>
        </div>
...
def save_note():
    ...
    # Save to WebDisk instead of local
    user_id = 1  # Default user
    passphrase = 'default'
    key = base64.urlsafe_b64encode(hashlib.sha256(passphrase.encode()).digest())
    fernet = Fernet(key)
    encrypted_data = fernet.encrypt(content.encode())
    new_file = File(filename=f'note_{uuid.uuid4()}.txt', data=encrypted_data, user_id=user_id, folder_id=None)
    db.session.add(new_file)
    db.session.commit()
    return jsonify({'note_id': new_file.id})
